...
# Outputs a JSON object with the command status and output.
...
if [ -z "$INSTANCE_ID" ] || [ -z "$SCRIPT_PATH" ]; then
  echo -e "${RED}Error: Instance ID and script path are required.${NC}" >&2
  exit 1
fi

if [ ! -f "$SCRIPT_PATH" ]; then
    echo -e "${RED}Error: Script path '$SCRIPT_PATH' is not a valid file.${NC}" >&2
    exit 1
fi
...
command_id_out=$(aws ssm send-command \
...
if [ $? -ne 0 ]; then
    ...
    log "AWS CLI Error: $command_id_out"
    exit 1
fi
COMMAND_ID=$command_id_out
...
# Get the final command output and print it to stdout
